...
If the user references an existing plan file or there is an obvious recent matching plan in `01_Memory/00_Context_LLM/05_Plans/`:
- Read it
...
Before asking planning questions, search `05_Scripts/07_Brain_Storming/` for files matching `*-requirements.md`.
...
- Institutional learnings from `01_Memory/00_Context_LLM/06_Solutions/`
...
- Build the filename following the repository convention: `01_Memory/00_Context_LLM/05_Plans/YYYY-MM-DD-NNN-<type>-<descriptive-name>-plan.md`
  - Create `01_Memory/00_Context_LLM/05_Plans/` if it does not exist
...
status: active
date: YYYY-MM-DD
origin: 05_Scripts/07_Brain_Storming/YYYY-MM-DD-<topic>-requirements.md  # include when planning from a requirements doc
...
- [Relevant `01_Memory/00_Context_LLM/06_Solutions/` insight]
...
- **Origin document:** [05_Scripts/07_Brain_Storming/YYYY-MM-DD-<topic>-requirements.md](path)
...
```text
01_Memory/00_Context_LLM/05_Plans/YYYY-MM-DD-NNN-<type>-<descriptive-name>-plan.md
```

Confirm:

```text
Plan written to 01_Memory/00_Context_LLM/05_Plans/[filename]
```
...
**Question:** "Plan ready at `01_Memory/00_Context_LLM/05_Plans/YYYY-MM-DD-NNN-<type>-<name>-plan.md`. What would you like to do next?"
...
Based on selection:
- **Open plan in editor** → Open `01_Memory/00_Context_LLM/05_Plans/<plan_filename>.md` using the current platform's file-open or editor mechanism (e.g., `open` on macOS, `xdg-open` on Linux, or the IDE's file-open API)
...
- **Share to Proof** → Upload the plan:
  ```bash
  CONTENT=$(cat 01_Memory/00_Context_LLM/05_Plans/<plan_filename>.md)
...
When the user selects "Create Issue", detect their project tracker from `01_Personal_Os/11_AGENTS.md` or, if needed for compatibility, `CLAUDE.md`:
...
4. If no tracker is configured:
   ...
   - Suggest adding the tracker to `01_Personal_Os/11_AGENTS.md` for future runs
