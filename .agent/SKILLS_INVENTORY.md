# SKILLS INVENTORY — Think Different PersonalOS v4.7

> 📍 **Source:** `01_Personal_Os/01_Core/02_Tools/02_Skills/` (canonical)
> 📍 **Backup activo:** `.agent/02_Skills/` (sincronizado)
> 📦 **Legacy archivado:** `01_Personal_Os/05_Archive/10_Skills_Legacy/`

---

## 📁 Estructura Activa: `.agent/02_Skills/` (15 directorios, 504 SKILL.md)

### 00_* — Infraestructura (no sincronizados, preservados)

| Directorio | SKILL.md | Propósito |
|-----------|----------|-----------|
| `00_Agent_Teams_Lite` | 13 | SDD sub-agents (explore, propose, spec, design, tasks, apply, verify, archive) |
| `00_Compound_Engineering` | 63 | Compound Engineering: skills de agentes, CE workflow, herramientas |
| `00_Engram` | 19 | Engram persistent memory system |
| `00_Invictus_Backup` | 78 | Backup de skills Invictus |
| `00_Personal_Os_Stack` | 1 | Stack de configuración del OS |
| `00_Skill_Auditor` | 1 | Auditoría de skills |

### 01-09 — Áreas Canónicas (sincronizadas desde source)

| Área | SKILL.md | Skills clave |
|------|----------|-------------|
| `01_Creacion_Contenidos` | 40 | SEO, copywriting, ads, analytics, content strategy, email, CRO |
| `02_Diseno_Ui_Ux` | 29 | UI/UX design, taste skill, brand identity, diagram studio, pencil design |
| `03_Video_Media` | 7 | Video production, media skills |
| `04_Automatizacion` | 37 | N8N (10 skills), automation, scripts |
| `05_Workflows` | 37 | E2E, playwright, testing, observability, edge cases |
| `06_Tools` | 112 | **Principal área técnica**: React, Next, Angular, Tailwind, TypeScript, Zod, Zustand, Django, Python, agent-native, testing, debugging, etc. |
| `07_Personal_Os` | 32 | Personajes/agentes (14), rituals, system, daily/weekly workflows |
| `08_Invictus_Web` | 15 | Invictus web design system, components, pages |
| `09_Claude_Ads` | 20 | Claude Ads platform skills |

---

## 🔍 Guía Rápida para el AI

### Necesito un skill de... → Buscar en

| Qué necesito | Dónde buscar |
|-------------|-------------|
| React / Next / Angular / Tailwind | `06_Tools/` |
| TypeScript / Zod / Zustand | `06_Tools/` |
| Python / Django / DRF / Pytest | `06_Tools/` |
| Playwright / Testing / Debugging | `06_Tools/` |
| SEO / Copywriting / Marketing | `01_Creacion_Contenidos/` |
| Diseño UI/UX / Branding | `02_Diseno_Ui_Ux/` |
| Video / Media | `03_Video_Media/` |
| Automatización / N8N | `04_Automatizacion/` |
| Workflows / E2E / Testing | `05_Workflows/` |
| Agentes / Tool building / Architecture | `06_Tools/` |
| Rituals / Daily / Weekly / Sync | `07_Personal_Os/` |
| Invictus Web | `08_Invictus_Web/` |
| Claude Ads | `09_Claude_Ads/` |
| Compound Engineering (CE) | `00_Compound_Engineering/` |
| SDD sub-agents | `00_Agent_Teams_Lite/` |
| Engram memory | `00_Engram/` |

### Skills únicos en Legacy Archive

Si el skill no está en las áreas activas, buscar en:
`01_Personal_Os/05_Archive/10_Skills_Legacy/`

Skill lookup: `find 01_Personal_Os/05_Archive/10_Skills_Legacy/ -name "<skill_name>" -type d`

---

## 📦 Legacy Archive: `10_Skills_Legacy/` (24 directorios, ~490 SKILL.md)

| Directorio | SKILL.md | Contenido |
|-----------|----------|-----------|
| `02_Project_Manager` | 8 | Morning standup, backlog, rituals |
| `03_Product_Manager` | 8 | Brainstorming, planning, PRD, Jira |
| `04_Product_Design` | 12 | Taste skill, minimalism, redesign, Dieter Rams |
| `05_Vibe_Coding` | 22 | React 19, Next 15, Tailwind 4, Angular, MCP (versiones v3) |
| `06_Testing` | 17 | TDD, debugging, coverage, E2E (versiones v3) |
| `07_DevOps` | 12 | Vercel, Supabase, MCP, observability (versiones v3) |
| `08_Personal_Os` | 20 | Fork terminal, orchestration, system guardian (10 UNICOS) |
| `09_Marketing` | 38 | Marketing, CRO, ads, image studio (10 UNICOS) |
| `10_Backup` | **205** | **Mega-archive de skills multi-versión (163 UNICOS)** |
| `11_Doc_Processing` | 3 | Document processing |
| `12_N8N` | 7 | N8N skills (7 UNICOS) |
| `13-22` | varios | Skills especializados varios |

## 📊 Totales

| Ubicación | SKILL.md | Notas |
|-----------|----------|-------|
| Source (`01_Core/02_Tools/02_Skills/`) | 394 | Canonical |
| Backup activo (`.agent/02_Skills/`) | 504 | Incluye 00_* extras + source sync |
| Legacy archive (`05_Archive/10_Skills_Legacy/`) | ~490 | Preservado completo |
| **Total general único estimado** | **~700+** | Skills únicos en todo el OS |

---
*Generado: 2026-05-23 | Actualizar tras cambios significativos en source*
