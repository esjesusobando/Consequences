...
def run_analysis(csv_path, output_dir):
    ...
    try:
        with open(peek_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"**Registros Totales:** {len(df)} | **Columnas:** {len(df.columns)}\n\n")
            ...
            f.write(f"- **Métricas de Negocio:** {', '.join(metric_cols) if metric_cols else 'Ninguna'}\n")
            f.write(f"- **Ratios Calculados:** {', '.join(ratios.keys()) if ratios else 'Ninguno'}\n")
            f.write(f"- **Segmentos Categóricos:** {', '.join(cat_cols) if cat_cols else 'Ninguno'}\n")
            f.write(f"- **IDs Técnicos (Excluidos):** {', '.join(id_cols) if id_cols else 'Ninguno'}\n")
...
def generate_dashboards(df, output_dir, num_cols, cat_cols):
    ...
    try:
        ...
        sns.kdeplot(data=df, x=main_metric, fill=True, color='#4A90E2', clip=(0, None), bw_adjust=1.0)
        ...
        save_plot('02_numeric_distribution.png', f'02. Success Density: {main_metric} Intensity')
    ...
    try:
        if cat_cols and num_cols:
            ...
            intensity_metric = 'clicks' if 'clicks' in df.columns else (num_cols[0] if num_cols else None)

            if intensity_metric:
                plot_data = df.groupby(cat)[intensity_metric].sum().sort_values(ascending=False).reset_index()
                total = plot_data[intensity_metric].sum()
                plot_data['share'] = (plot_data[intensity_metric] / total) * 100

                bars = plt.bar(plot_data[cat], plot_data['share'],
                ...
                plt.title(f'03. Market Fragmentation: {cat} Share by {intensity_metric}')
            else:
                sns.countplot(data=df, x=cat, palette='viridis')

            plt.xticks(rotation=45)
            save_plot('03_categorical_distribution.png', f'03. Market Fragmentation by {cat}')
    except Exception as e: errors.append(f"03: {str(e)}")

    # 4. Anomaly Radar (Z-Score Rigor v4.1.5)
    try:
        numeric_df = df[num_cols].select_dtypes(include=[np.number])
        if not numeric_df.empty:
            plt.figure(figsize=(12, 6))
            # Calcular Z-Scores absolutos (manejar std=0 para evitar NaNs)
            stds = numeric_df.std()
            z_scores = np.abs((numeric_df - numeric_df.mean()) / stds.replace(0, 1)).fillna(0)
            mean_abs_z = z_scores.mean()

            bars = plt.bar(mean_abs_z.index, mean_abs_z, color=sns.color_palette('viridis', len(mean_abs_z)))
            plt.axhline(1.0, color='red', linestyle='--', alpha=0.5, label='Typical Variance')

            # Etiquetas de valor precisas
            for bar in bars:
                height = bar.get_height()
                plt.text(bar.get_x() + bar.get_width()/2., height + 0.02, f'{height:.2f}',
                         ha='center', va='bottom', fontsize=9, fontweight='bold')

            plt.title('04. Strategic Anomaly Detection: Mean Absolute Z-Score')
            plt.ylabel('Mean Abs Z-Score (Deviance)')
            plt.xticks(rotation=45)
            plt.legend()
    ...
    try:
        if cat_cols and num_cols:
            ...
            bars = plt.bar(plot_data[cat_cols[0]], plot_data[target],
            ...
            plt.xticks(rotation=45)
            plt.ylabel(target)
            save_plot('05_cat_vs_num_bar.png', f'05. Efficiency Benchmark: {target} by {cat_cols[0]}')
    ...
    try:
        plt.figure(figsize=(12, 6))
        sns.lineplot(data=df, x=df.index, y=num_cols[0], marker='o', linewidth=2.5, color='#E67E22')
    ...
    try:
        if cat_cols and num_cols:
            ...
            plot_data = df.groupby(cat)[val].sum().sort_values(ascending=False).reset_index()
    ...
    try:
        if cat_cols:
            ...
            dominance_metric = 'spend' if 'spend' in df.columns else ('clicks' if 'clicks' in df.columns else None)

            if dominance_metric:
                plot_data = df.groupby(cat)[dominance_metric].sum().sort_values(ascending=False)
...
if __name__ == '__main__':
    run_analysis(r'C:\Users\sebas\Downloads\01 Revisar\07 Now\personal-os-main\personal-os-main\.claude\knowledge\Skill\43_health-data-analyst\examples\02_Ventas\test_sales_data.csv', r'C:\Users\sebas\Downloads\01 Revisar\07 Now\personal-os-main\personal-os-main\.claude\knowledge\Skill\43_health-data-analyst\examples\02_Ventas')
