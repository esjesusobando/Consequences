...
Read files in parallel batches where there are no dependencies between them. For example, batch README.md, entry points, and 01_Personal_Os/11_AGENTS.md/CLAUDE.md together in a single turn since none depend on each other's content.
...
5. **01_Personal_Os/11_AGENTS.md or CLAUDE.md** (if exists) -- for project conventions and patterns already documented.
