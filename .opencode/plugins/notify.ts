...
async function sendNodeNotification(options: NotificationOptions): Promise<void> {
	...
	if (process.platform === "darwin") {
		const activate = terminalInfo.bundleId
			? `; tell application id "${terminalInfo.bundleId}" to activate`
			: ""
		await spawnDetached([
			"osascript",
			"-e",
			`display notification "${message.replaceAll('"', '\\"')}" with title "${title.replaceAll('"', '\\"')}" sound name "${sound.replaceAll('"', '\\"')}"${activate}`,
		])
		return
	}
