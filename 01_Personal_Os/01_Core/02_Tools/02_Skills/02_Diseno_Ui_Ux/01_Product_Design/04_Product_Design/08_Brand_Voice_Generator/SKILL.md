...
## Esencia Original
> **Propósito:** Generar archivos completos de sistema de marca (brand.json, config.json, brand-system.md, tone-of-voice.md) mediante proceso interactivo.
> **Flujo:** Preguntar brand info → Generar tokens → Definir voice → Crear archivos

## ⚠️ Gotchas (Errores Comunes a Evitar)

- **[ERROR]**: No preguntar por la audiencia objetivo
  - **Por qué**: Brand voice depende de a quién le hablás
  - **Solución**: Siempre preguntar: quién es el usuario, qué le importa

- **[ERROR]**: Generar brand genérico
  - **Por qué**: No diferencia al usuario de otros
  - **Solución**: Preguntar por unique selling points, valores

- **[ERROR]**: Olvidar los archivos de output
  - **Por qué**: El usuario no tiene dónde guardar/configurar
  - **Solución**: SIEMPRE crear: brand.json, config.json, brand-system.md, tone-of-voice.md

- **[ERROR]**: No verificar location de output
  - **Por qué**: Archivos se pierden en wrong directory
  - **Solución**: Preguntar dónde guardar o usar default

## 📁 Progressive Disclosure

> Para información detallada:
- [references/brand-schema.md](references/brand-schema.md) — Schema de brand.json
- [references/voice-questions.md](references/voice-questions.md) — Preguntas interactivas

## 🛠️ Scripts

- [scripts/generate-brand.py](scripts/generate-brand.py) — Genera esqueleto de brand files

## 💾 State Persistence

Guardar brand files en:
- `03_Knowledge/08_Config/brand/`
- O en directorio especificado por usuario
...
```
01_Personal_Os/01_Core/02_Tools/02_Skills/02_Diseno_Ui_Ux/07_Premium_Image_Studio/brands/{brand-name}/
```
...
1. **List created files:**
   ```
   Glob: 01_Personal_Os/01_Core/02_Tools/02_Skills/02_Diseno_Ui_Ux/07_Premium_Image_Studio/brands/{brand-name}/*
   ```

2. **Remind about assets:**
   > "Don't forget to copy your logo file to `01_Personal_Os/01_Core/02_Tools/02_Skills/02_Diseno_Ui_Ux/07_Premium_Image_Studio/brands/{brand-name}/assets/`"
