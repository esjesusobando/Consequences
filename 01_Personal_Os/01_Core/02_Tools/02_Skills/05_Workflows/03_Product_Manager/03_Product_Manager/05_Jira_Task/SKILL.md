...
name: jira-task
description: >
  Creates Jira tasks following standard format with MCP integration.
  Triggers on: create task, jira task, ticket, issue, bug report, feature request.
license: Apache-2.0
metadata:
...
## Esencia Original
> **Propósito:** Crear tasks de Jira bien estructurados con splitting multi-component (bugs como siblings, features como parent+children).
> **Flujo:** Definir tipo → Template → MCP create → Link dependencies

## ⚠️ Gotchas (Errores Comunes a Evitar)

- **[ERROR]**: Task multi-component sin splitting
  - **Por qué**: Difícil de asignar, rastrear y hacer code review
  - **Solución**: Bugs = tasks separados; Features = parent + children

- **[ERROR]**: Olvidar customfield_10359 (Team)
  - **Por qué**: Jira rechaza el issue
  - **Solución**: Siempre incluir UI/API/SDK

- **[ERROR]**: Title sin formato [TYPE] description (component)
  - **Por qué**: Difícil filtrar y priorizar
  - **Solución**: Usar [BUG], [FEATURE], [ENHANCEMENT], [REFACTOR]

- **[ERROR]**: Acceptance Criteria no testeables
  - **Por qué**: No se puede verificar que está done
  - **Solución**: Específicos y medibles

## 📁 Progressive Disclosure

> Para información detallada:
- [references/task-template.md](references/task-template.md) — Templates de tasks
- [references/title-conventions.md](references/title-conventions.md) — Convenciones de títulos

## 🛠️ Scripts

- [scripts/create-task.py](scripts/create-task.py) — Genera estructura de task

## 💾 State Persistence

Guardar tasks en:
- `04_Docs/tasks/YYYY-MM-DD-<task-name>.md`
- Copiar a Jira via MCP
