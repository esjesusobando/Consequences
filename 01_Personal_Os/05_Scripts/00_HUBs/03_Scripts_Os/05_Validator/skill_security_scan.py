...
class SecurityScanner:
    ...
    def _check_line(self, file_path: Path, line_num: int, line: str):
        """Verifica una línea contra los patrones - BLINDADO contra bypass."""
        # BLINDO: Normalizar la línea antes de buscar
        # Eliminar comments que podrían evadir detección
        line_normalized = line.strip()
        
        # Skip comments que no son relevantes para security
        if line_normalized.startswith("#") or line_normalized.startswith("//"):
            return
            
        for severity, patterns in self.PATTERNS.items():
            for pattern, message in patterns:
                # BLINDO: usar search más restrictivo con timeout
                try:
                ...
                if match:
                    # BLINDO: Verificar que no sea un falso positivo en string/comment
                    # Buscar si está dentro de string o comentario
                    if "#" in line and '"' not in line.split("#")[0]:
                        continue  # Es un comment, skip
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python skill_security_scan.py --skill 01_Personal_Os/00_Core/02_Tools/02_Skills/25_Mi_Skill
  python skill_security_scan.py --skill 01_Personal_Os/00_Core/02_Tools/02_Skills/25_Mi_Skill --verbose
  python skill_security_scan.py --skill 01_Personal_Os/00_Core/02_Tools/02_Skills/25_Mi_Skill --output report.txt
        """,
    )
