...
sys.path.insert(0, str(SCRIPT_DIR.parent))  # 03_Scripts_Os
...
def validate_arsenal_integrity() -> bool:
    ...
    Usa config_paths para resolver paths dinámicamente.
    """
    print(f"\n{INFO}[SCAN] Validando Integridad del Arsenal (Skills)...{RESET}")

    # Rutas de búsqueda - USAR config_paths
    skills_paths = [SKILLS_DIR]
    # Agregar subdirectorios de skills si existen
    if SKILLS_DIR.exists():
        for sub in SKILLS_DIR.iterdir():
            if sub.is_dir():
                skills_paths.append(sub)

    inventory_file = INVENTORY_FILE

    if not os.path.exists(inventory_file):
        print(f"{WARNING}[SKIP] No se encontró el inventario para validar arsenal: {inventory_file}{RESET}")
        return True
    ...
    # Buscar patrones de skills: [NN_Nombre_Skill] o [Nombre_Skill]
    # Filtrar para que no coincida con archivos de documentación
    ...
    for skill_name in skill_refs:
        ...
        if not found:
            # Buscar en cualquier subdir de SKILLS_DIR
    ...
    if missing:
        ...
        for m in missing[:10]:  # Limit display
            print(f"  - {m}")
        if len(missing) > 10:
...
def validate_integrations() -> bool:
    """Verifica el estado de integraciones críticas del sistema."""
    ...
    # Recap Planning - buscar en 10_Legacy (ubicación correcta)
    recap_path = ENGINE_DIR / "10_Legacy" / "38_Recap_Planning.py"
    if not recap_path.exists():
        recap_path = ENGINE_DIR / ".backup" / "10_Legacy_backup_20260420" / "38_Recap_Planning.py"
    ...
    return True  # No fallar por integraciones opcionales
