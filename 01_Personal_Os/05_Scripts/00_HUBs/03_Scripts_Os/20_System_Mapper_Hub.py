...
Usage:
    ...
    python 16_System_Mapper_Hub.py --sync-rules  # genera copia .agent del audit rule

Outputs (escritos en 01_Personal_Os/05_Scripts/02_Agent_Teams_Lite/00_Manifest/):
...
def scan_skills():
    ...
    return {
        "version": "v3.0",
        "generated": datetime.now().isoformat(timespec="seconds"),
        "base_path": "01_Personal_Os/00_Core/02_Tools/02_Skills/",
        "totals": {
            "areas": len(by_area),
...
def scan_hubs():
    ...
    return {
        "version": "v3.0",
        "generated": datetime.now().isoformat(timespec="seconds"),
        "base_path": "01_Personal_Os/05_Scripts/00_HUBs/03_Scripts_Os/",
        "totals": {
...
def scan_hooks():
    ...
    return {
        "version": "v3.0",
        "generated": datetime.now().isoformat(timespec="seconds"),
        "base_path": "01_Personal_Os/00_Core/02_Tools/05_Hooks/",
        "totals": {
            "phases": len(by_phase),
...
def scan_inventory():
    ...
    return {
        ...
        "agents": {
            "definition": "archivos .md en 00_Core/02_Tools/01_Agents/ (excluye README, templates, references)",
...
def validate():
    ...
    # Drift detection between Core and .agent
    ...
    if core_rules.exists() and agent_rules.exists():
        ...
        if abs(core_lines - agent_lines) > 10:
            print(f"\n  💡 Drift detectado: Core tiene {core_lines} líneas, .agent tiene {agent_lines}")
...
def sync_rules() -> int:
    ...
    # Step 1: Substitute metric values (column 2 only)
    value_subs = [
        (r'(\|\s*\*\*HUBs\*\*\s*\|\s*\*\*)\d+(\*\*)',
         rf'\g<1>{totals["hubs"]}\g<2>'),
        (r'(\|\s*\*\*Scripts\*\*\s*\|\s*\*\*)\d+(\s*\(.*?\)\s*\*\*)',
         rf'\g<1>{hubs["scripts_totales"]}\g<2>'),
        (r'(\|\s*\*\*Skills\*\*\s*\|\s*\*\*)\d+(\s*\(.*?\)\s*\*\*)',
         rf'\g<1>{totals["skills"]}\g<2>'),
        (r'(\|\s*\*\*MCPs\*\*\s*\|\s*\*\*)(.+?)(\*\*)',
         rf'\g<1>{totals["mcps_claude"]} (Claude) + {totals["mcps_opencode"]} (OpenCode)\g<3>'),
        (r'(\|\s*\*\*Agentes source\*\*\s*\|\s*\*\*)\d+(\*\*)',
         rf'\g<1>{totals["agents_source"]}\g<2>'),
        (r'(\|\s*\*\*Workflows\*\*\s*\|\s*\*\*)\d+(\s*\(.*?\)\s*\*\*)',
         rf'\g<1>{totals["workflows"]}\g<2>'),
        (r'(\|\s*\*\*Hooks\*\*\s*\|\s*\*\*)\d+(\s*\(.*?\)\s*\*\*)',
         rf'\g<1>{totals["hooks"]}\g<2>'),
        (r'(\|\s*\*\*Rules \(\.mdc\)\*\*\s*\|\s*\*\*)\d+(\*\*)',
         rf'\g<1>{totals["rules"]}\g<2>'),
    ]

    output = template
    for pattern, replacement in value_subs:
        output = re.sub(pattern, replacement, output)

    # Step 2: Update date column (column 3) to today
    # Match: | <date> ✅ | at end of metric rows
    output = re.sub(
        r'(\|\s*)[\d]{4}-[\d]{2}-[\d]{2}\s*✅\s*(\|)',
        rf'\g<1>{date_col}\g<2>',
        output
    ...
    # Inject auto-generated warning after YAML frontmatter
    ...
    # Write to .claude/02_Rules/ (if parent dir exists)
...
def main():
    ...
    if not (args.scan or args.validate or args.report or args.sync_rules):
        parser.print_help()
        return 1
