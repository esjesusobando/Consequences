...
|---------------------------------|-----------------|-------------------|---------------------------------------------|----------------------------------------------------|
| **engram**                       | 2.7k ⭐           | v2.10+             | `08_Engram/` + `25_engram/`                  | Memoria persistente (backbone OS)                   |
| **gentle-ai**                    | 2.2k ⭐           | v1.21.0            | `10_Gentle_AI/`                              | AI Gentle Stack (reemplaza ATL)                     |
| **gentle-pi**                    | —                | v0.3.2             | `24_gentle-pi/`                              | Gentleman harness para Pi (SDD + subagents)         |
| **gentle-engram**                | —                | v0.1.4             | pnpm global + `25_engram/plugin/pi/`         | Memoria persistente para Pi coding agent            |
| **taste-skill**                  | 3.9k ⭐           | —                  | `22_Taste_Skill/`                            | Diseño premium UI/UX                                |
| **Gentleman.Dots**               | 1.7k ⭐           | v2.9.10            | `11_Gentleman_Dots/`                         | LazyVim config premium                              |
| **gentleman-guardian-angel**     | 961 ⭐            | v2.8+              | `12_Gentleman_Guardian_Angel/`               | GGA Code Review automático                          |
| **frontend-slides**              | —                | v2.0.0             | `09_Frontend_Slides/`                        | Presentaciones HTML                                 |
...
| Herramienta              | Config                                   | Skills / Docs                                                |
|-------------------------|-----------------------------------------|-------------------------------------------------------------|
| **Claude Code**          | `.mcp.json` (raíz)                       | `01_Personal_Os/01_Core/02_Tools/02_Skills/`                 |
| **OpenCode**             | `~/.config/opencode/`                    | Skills locales en `.codex/`                                  |
| **Pi (gentle-pi)**       | `PNPM_HOME`, `MINIMAX_API_KEY` vars      | gentle-pi skill harness + SDD phases                         |
| **GGA**                  | `.agent/05_GGA/`                         | `.agent/05_GGA/bin/gga run`                                  |
| **Taste-Skills**         | Cursor IDE                               | `.cursor/02_Skills/11_Taste_Skills/`                         |
| **Taste-Skills**         | OpenCode                                 | `01_Personal_Os/01_Core/02_Tools/02_Skills/`                 |
| **Engram**               | MCP server                               | `25_engram/` + plugin `08_Engram/`                           |

---

## 🛠️ gentle-pi — Setup (Gentleman harness para Pi)

gentle-pi es un plugin que corre sobre `@earendil-works/pi-coding-agent`. No es un CLI standalone.

```bash
# Install pnpm
npm install -g pnpm

# Install pi + gentle-pi
pnpm install -g @earendil-works/pi-coding-agent
pnpm install -g gentle-pi

# Run (desde directorio con permisos de escritura)
cd Think_Different
pi --provider minimax --model MiniMax-M2.7
```

Paquetes instalados:
- `pnpm 11.1.2` → `AppData\Local\pnpm`
- `@earendil-works/pi-coding-agent 0.74.0` → pnpm global store
- `gentle-pi 0.3.2` → npm global `AppData\Roaming\npm`
- `gentle-engram 0.1.4` → pnpm global store
- `yt-dlp 2026.03.17` → `Python\Python314\Scripts`
- `youtube-transcript-api 1.2.4` → pip

Variables necesarias (ya en `.bashrc`):
```
MINIMAX_API_KEY, MINIMAX_GROUP_ID, PNPM_HOME, PI_CODING_AGENT_DIR
```

Para transcripción YouTube:
```python
