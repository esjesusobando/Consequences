...
| `compound-engineering:review:project-standards-reviewer`                            | CLAUDE.md and 01_Personal_Os/11_AGENTS.md compliance -- frontmatter, references, naming, portability                        |
...
1. Use the native file-search tool (e.g., Glob in Claude Code) to find all `**/CLAUDE.md` and `**/01_Personal_Os/11_AGENTS.md` in the repo.
2. Filter to those whose directory is an ancestor of at least one changed file. A standards file governs all files below it (e.g., `plugins/compound-engineering/01_Personal_Os/11_AGENTS.md` applies to everything under `plugins/compound-engineering/`).
