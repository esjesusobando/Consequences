...
- Orchestrator skill reads multiple files, concatenates their contents into a block (e.g., `<standards>` with full CLAUDE.md/01_Personal_Os/11_AGENTS.md content), and injects it into the sub-agent prompt
...
Having the orchestrator read all relevant file contents and pass them in a content block. This was the initial approach for the `project-standards-reviewer` agent in ce:review: Stage 3b collected all CLAUDE.md/01_Personal_Os/11_AGENTS.md content into a `<standards>` block passed in the sub-agent prompt.
...
1. Stage 3b: orchestrator globs for CLAUDE.md/01_Personal_Os/11_AGENTS.md paths in changed directories, emits a `<standards-paths>` block
...
Empirical testing showed that how the skill phrases a search instruction has a dramatic effect on tool call count. For the same task (find ancestor CLAUDE.md/01_Personal_Os/11_AGENTS.md files for changed paths):
