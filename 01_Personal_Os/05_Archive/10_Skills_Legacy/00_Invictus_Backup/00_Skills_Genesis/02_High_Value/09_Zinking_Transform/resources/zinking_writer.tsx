...
const ZinkingWriter = () => {
  ...
  const analogies = [
    { concept: "proceso", analogy: "afinar un instrumento - no lo haces una vez y olvidas; es un ajuste constante que requiere oído atento" },
    { concept: "simplificar", analogy: "ir de mochilero en lugar de en casa rodante - te mueves más rápido, llegas a lugares inaccesibles, y la experiencia se vuelve íntima" },
    { concept: "aprender", analogy: "cocinar sin receta - al principio necesitas medir todo, pero con el tiempo desarrollas intuición" },
    { concept: "editar", analogy: "tallar una escultura - no agregas material, quitas lo que sobra hasta revelar la forma que siempre estuvo ahí" },
    { concept: "práctica", analogy: "cultivar un jardín - no ves el crecimiento diario, pero de repente florece algo hermoso" }
  ];
  ...
  const transformToZinking = (text, type) => {
    ...
    const opening = zinkingPatterns.openings[Math.floor(Math.random() * zinkingPatterns.openings.length)];
    ...
    transformed += sensorialDetails[Math.floor(Math.random() * sensorialDetails.length)];
    transformed += '\n\n';

    // 2. TRANSICIÓN REFLEXIVA
    const transition = zinkingPatterns.transitions[Math.floor(Math.random() * zinkingPatterns.transitions.length)];
    ...
    const mainContent = lines.map(line => {
      // Eliminar bullet points y números
      let cleaned = line.replace(/^[\d\-\*\•]+\.?\s*/, '');
      
      // Hacer más conversacional
      cleaned = cleaned.replace(/debes|tienes que|es necesario/gi, 'podrías considerar');
      cleaned = cleaned.replace(/siempre|nunca/gi, 'a menudo');
      cleaned = cleaned.replace(/^Los expertos|^Se recomienda/gi, 'He descubierto que');
    ...
    const validation = zinkingPatterns.validations[Math.floor(Math.random() * zinkingPatterns.validations.length)];
    ...
    const invitation = zinkingPatterns.invitations[Math.floor(Math.random() * zinkingPatterns.invitations.length)];
    const closing = zinkingPatterns.closings[Math.floor(Math.random() * zinkingPatterns.closings.length)];
    ...
    transformed += reflectiveQuestions[Math.floor(Math.random() * reflectiveQuestions.length)];
  ...
  const examples = {
    educativo: "La gestión del tiempo es importante para la productividad.\nDebes planificar tus tareas.\nUsa listas de pendientes.\nElimina distracciones.",
    inspiracional: "El éxito requiere esfuerzo.\nNo te rindas.\nPersigue tus metas.",
    tecnico: "Para optimizar el código:\n- Usa variables descriptivas\n- Evita repetición\n- Comenta tu código"
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-sm text-gray-500 mt-2 italic">
            "La magia sucede cuando la estructura se vuelve invisible y solo queda la verdad"
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              { value: 'educativo', icon: BookOpen, label: 'Educativo/Tutorial' },
              { value: 'inspiracional', icon: Heart, label: 'Inspiracional' },
              { value: 'tecnico', icon: Lightbulb, label: 'Técnico/Explicativo' }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-800">Contenido Original</h2>
              <button
                onClick={loadExample}
          ...
          {/* Output */}
          <div className="bg-white rounded-2xl shadow-lg p-6">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">Versión Zinking</h2>
              ...
              {output ? (
                ...
                <div className="h-full flex items-center justify-center text-gray-400 text-center">
                  <div>
                    ...
                    <p className="text-sm mt-2">Con calidez, autenticidad y alma.</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="mt-8 bg-white rounded-2xl shadow-lg p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Principios de Transformación</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { title: 'Vulnerabilidad', desc: 'Compartir procesos, no solo éxitos' },
              { title: 'Analogías', desc: 'Comparar con experiencias universales' },
              { title: 'Validación', desc: 'Reconocer lo que el receptor siente' },
              { title: 'Simplificación', desc: 'Reducir al núcleo esencial' },
              { title: 'Presencia', desc: 'Invitar al momento presente' },
              { title: 'Gratitud', desc: 'Enmarcar como privilegio, no derecho' }
            ].map((principle, idx) => (
              <div key={idx} className="p-4 bg-orange-50 rounded-xl">
                <h4 className="font-semibold text-orange-800 mb-1">{principle.title}</h4>
