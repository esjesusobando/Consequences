...
3. **Follow Existing Patterns**
   ...
   - Follow project coding standards (see 01_Personal_Os/11_AGENTS.md; use CLAUDE.md only if the repo still keeps a compatibility shim)
...
1. **Run Core Quality Checks**
   ...
   # Run linting (per 01_Personal_Os/11_AGENTS.md)
...
2. **Build prompt** — For each task, assemble a prompt from the plan's implementation unit (Goal, Files, Approach, Conventions from project CLAUDE.md/01_Personal_Os/11_AGENTS.md). Include rules: no git commits, no PRs, run `git status` and `git diff --stat` when done. Never embed credentials or tokens in the prompt - pass auth through environment variables.
