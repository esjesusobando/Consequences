...
execute_opencode() {
  ...
  # Use stdin to avoid ARG_MAX limits on Windows with large prompts (many files)
  if [[ -n "$model" ]]; then
...
execute_provider_with_timeout() {
  ...
  case "$base_provider" in
    ...
    opencode)
      ...
      if [[ -n "$model" ]]; then
        execute_with_timeout "$timeout" "OpenCode" bash -c "printf '%s' \"\$1\" | opencode run --model \"$model\" 2>&1" -- "$prompt"
      else
        execute_with_timeout "$timeout" "OpenCode" bash -c "printf '%s' \"\$1\" | opencode run 2>&1" -- "$prompt"
      fi
      ;;
    ollama)
      local model="${provider#*:}"
