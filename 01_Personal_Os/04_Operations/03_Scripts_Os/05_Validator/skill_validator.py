...
class SkillValidator:
    ...
    def _read_skill_content(self) -> tuple[str, dict]:
        """Read SKILL.md and parse YAML frontmatter - BLINDADO contra injection."""
        if not self.skill_md_path.exists():
            return "", {}

        content = self.skill_md_path.read_text(encoding="utf-8")

        # Parse YAML frontmatter - BLINDADO: usar safe_load con límites
        frontmatter = {}
        if content.startswith("---"):
            parts = content.split("---", 2)
            if len(parts) >= 3:
                try:
                    # BLINDO: usar safe_load con objeto limitado a tipos simples
                    # NO permitir objetos Python arbitrarios
                    import yaml
                    # Usar SafeLoader por defecto para evitar object deserialization
                    frontmatter = yaml.load(parts[1], Loader=yaml.SafeLoader) or {}
                    content = parts[2].strip()
                except (yaml.YAMLError, yaml.constructor.ConstructorError):
                    # Si falla SafeLoader, intentar con SafeLoader restringido
                    try:
