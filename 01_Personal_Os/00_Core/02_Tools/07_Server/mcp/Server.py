...
SCRIPTS_DIR = Path(__file__).parent.parent.parent / "05_Scripts/03_Scripts_Os"
...
# Import dynamic paths from config_paths
from config_paths import TASKS_DIR, EVALS_DIR, BASE_DIR, ROOT_DIR
...
# Configuration - use config_paths for dynamic paths
# TASKS_DIR, EVALS_DIR, BASE_DIR ya vienen de config_paths
...
def find_similar_tasks(
    item: str, existing_tasks: List[Dict[str, Any]], config: dict = DEDUP_CONFIG
) -> List[Dict[str, Any]]:
    ...
    for task in existing_tasks:
        ...
        if item_keywords and task_keywords:
            keyword_overlap = len(item_keywords & task_keywords) / len(
                item_keywords | task_keywords
            )
        else:
            keyword_overlap = 0
...
def generate_clarification_questions(item: str) -> List[str]:
    ...
    if any(word in item_lower for word in ["fix", "bug", "error", "issue"]):
        questions.append(
            "Which specific bug or error? Can you provide more details or error messages?"
        )
    ...
    # Missing target
    if any(
        word in item_lower for word in ["email", "contact", "reach out", "follow up"]
    ):
    ...
    if not questions:
        questions.append(
            "Can you provide more specific details about what needs to be done?"
        )
...
def guess_category(item: str) -> str:
    ...
    if any(
        word in item_lower
        for word in ["email", "contact", "reach out", "follow up", "meeting", "call"]
    ):
        return "outreach"
    elif any(
        word in item_lower
        for word in ["code", "api", "database", "deploy", "fix", "bug", "implement"]
    ):
        return "technical"
    elif any(
        word in item_lower
        for word in ["research", "study", "learn", "understand", "investigate"]
    ):
        return "research"
    elif any(
        word in item_lower
        for word in ["write", "draft", "document", "blog", "article", "proposal"]
    ):
        return "writing"
    elif any(
        word in item_lower
        for word in ["expense", "invoice", "schedule", "calendar", "organize"]
    ):
        return "admin"
    elif any(
        word in item_lower
        ...
        types.Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "status": {
                        "type": "string",
                        "description": "Filter by status (n,s,b,d)",
                    },
                    "include_done": {
                        "type": "boolean",
        ...
        types.Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "judgement": {
                        "type": "string",
                        "description": "Filter by judgement (pending,success,partial,failure)",
                    },
                    "limit": {
                        "type": "integer",
                        "description": "Max evals to return",
                        "default": 20,
                    },
                },
            },
        ...
        types.Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "session_id": {
                        "type": "string",
                        "description": "Session ID or 'recent'",
                    }
                },
            },
        ),
        types.Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "judgement": {
                        "type": "string",
                        "description": "Judgement (success,partial,failure)",
                    },
...
async def handle_call_tool(
    ...
    if name == "list_tasks":
        ...
        if not filepath.exists():
            result = {"success": False, "error": f"Task file not found: {task_file}"}
        else:
        ...
        if not backlog_file.exists():
            result = {"success": False, "error": "BACKLOG.md not found"}
        else:
            with open(backlog_file, "r") as f:
                content = f.read().strip()
        ...
        try:
            ...
            result = {"success": True, "message": "Backlog cleared successfully"}
        except Exception as e:
            result = {"success": False, "error": str(e)}
                ...
                result["potential_duplicates"].append(
                    {
                        ...
                        if similar_tasks[0]["similarity_score"] > 0.8
                        else "review",
                    }
                )
        ...
        try:
            ...
            session_id = (
                arguments.get("session_id", "recent") if arguments else "recent"
            )
