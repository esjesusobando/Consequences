...
> **Metaskill**: Sistema de orquestación multi-agente para Product Management, ahora reemplazado por SDD + CE.

Esta skill es el **orquestador histórico** del PersonalOS. Aunque obsoleto, sus patrones se migraron a SDD y Compound Engineering.
...
| Let it Fail                                            | 05_Scripts/01_Auto_Improvement/                                                 |
...
## ⚠️ Gotchas (para referencia histórica)

### ERROR 1: Orquestación sin back pressure
- **Por qué**: Sin gates, los agentes siguen aunque no tengan contexto suficiente
- **Solución**: Implementar wait points antes de cada fase

### ERROR 2: No capturar instrucciones faltantes
- **Por qué**: Errores no se documentan y se repiten
- **Solución**: Always preguntar "¿qué instrucción falta?" al final

### ERROR 3: Git context desactualizado
- **Por qué**: Sin refresh, responde con info vieja
- **Solución**: Always refresh git context antes de responder
