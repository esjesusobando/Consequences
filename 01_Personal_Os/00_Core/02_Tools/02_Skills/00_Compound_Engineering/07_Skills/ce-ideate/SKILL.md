...
1. **Quick context scan** — dispatch a general-purpose sub-agent with this prompt:
   ...
   > Read the project's 01_Personal_Os/11_AGENTS.md (or CLAUDE.md only as compatibility fallback, then README.md if neither exists), then discover the top-level directory layout using the native file-search/glob tool (e.g., `Glob` with pattern `*` or `*/*` in Claude Code). Return a concise summary (under 30 lines) covering:
...
- **Past learnings** — relevant institutional knowledge from 01_Personal_Os/01_Memory/00_Context_LLM/06_Solutions/
