...
If the user references an existing plan file or there is an obvious recent matching plan in `05_Scripts/05_Plans/`:
- Read it
...
- Build the filename following the repository convention: `05_Scripts/05_Plans/YYYY-MM-DD-NNN-<type>-<descriptive-name>-plan.md`
  - Create `05_Scripts/05_Plans/` if it does not exist
...
```text
05_Scripts/05_Plans/YYYY-MM-DD-NNN-<type>-<descriptive-name>-plan.md
```

Confirm:

```text
Plan written to 05_Scripts/05_Plans/[filename]
```
...
**Question:** "Plan ready at `05_Scripts/05_Plans/YYYY-MM-DD-NNN-<type>-<name>-plan.md`. What would you like to do next?"
...
Based on selection:
- **Open plan in editor** → Open `05_Scripts/05_Plans/<plan_filename>.md` using the current platform's file-open or editor mechanism (e.g., `open` on macOS, `xdg-open` on Linux, or the IDE's file-open API)
...
- **Share to Proof** → Upload the plan:
  ```bash
  CONTENT=$(cat 05_Scripts/05_Plans/<plan_filename>.md)
