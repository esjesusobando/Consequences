...
If the user references an existing brainstorm topic or document, or there is an obvious recent matching `*-requirements.md` file in `05_Scripts/07_Brain_Storming/`:
- Read the document
...
*Constraint Check* — Check project instruction files (`01_Personal_Os/11_AGENTS.md`, and `CLAUDE.md` only if retained as compatibility context) for workflow, product, or scope constraints that affect the brainstorm. If these add nothing, move on.
...
Ensure `05_Scripts/07_Brain_Storming/` directory exists before writing.
...
```bash
CONTENT=$(cat 05_Scripts/07_Brain_Storming/YYYY-MM-DD-<topic>-requirements.md)
...
Requirements doc: 05_Scripts/07_Brain_Storming/YYYY-MM-DD-<topic>-requirements.md  # if one was created
