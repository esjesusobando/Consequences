...
Note that files in `01_Memory/00_Context_LLM/05_Plans/` and `01_Memory/00_Context_LLM/06_Solutions/` are compound-engineering pipeline artifacts created by `/workflows:plan`. They are intentional, permanent living documents — do not recommend their removal or characterize them as unnecessary.
