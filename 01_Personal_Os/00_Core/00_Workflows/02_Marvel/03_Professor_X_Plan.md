...
1. **Leer Reglas de Sesión**:
   - Leer `01_Personal_Os/00_Core/01_Rules/` — revisar cualquier regla con `alwaysApply: true`.

2. **Cargar Memoria de Largo Plazo (AGENTE)**:
   - Ejecutar `engram_mem_context(limit=10)` con project="Think_Different" para recuperar contexto previo.
   - Leer `01_Personal_Os/05_Scripts/00_Context_LLM/00_Context_Memory/` — mapa del sistema actualizado.

3. **Revisar Notas de Proceso (USUARIO)**:
   - Leer los archivos más recientes en `01_Personal_Os/05_Scripts/00_Context_LLM/01_Process_Notes/`.
