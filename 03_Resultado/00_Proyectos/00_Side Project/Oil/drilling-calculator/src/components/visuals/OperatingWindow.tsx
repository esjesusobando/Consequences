...
export function OperatingWindow({
  ...
  return (
    <div className="operating-window-v3">
      ...
      <div className="ow-header-row1">
        <div className="ow-title-group">
          <span className="ow-title">VENTANA OPERATIVA</span>
          <span className="ow-subtitle">Comparativo Triple Barra</span>
        </div>
      ...
      <div className="ow-container">
        ...
        {validFG === 0 && validECD === 0 && validMW === 0 ? (
          ...
          <>
            ...
            {/* ── Leyenda Silicon Valley (Pills) ── */}
            <div className="ow-legend">
              <div className="ow-legend-pills">
                {/* Lodo Pill */}
                <span
                  className="ow-pill"
                  style={{
                ...
                >
                  <span
                    className="ow-pill-dot"
                    style={{ background: "rgba(255,255,255,0.6)" }}
                  />
                  MW · {validMW.toFixed(2)}
                </span>
                {/* ECD Pill */}
                <span
                  className="ow-pill"
                  style={{
                    background: `${ecdColor}15`,
                    border: `1px solid ${ecdColor}40`,
                    color: ecdColor,
                  }}
                >
                  <span
                    ...
                    style={{ background: ecdColor }}
                  />
                  ECD · {validECD.toFixed(2)}
                  <span
                    className={`ow-status-dot ${isDangerous ? "danger" : "ok"}`}
                  >
                    {isDangerous ? "CRÍTICO" : "SEGURO"}
                  </span>
                </span>
                {/* Fractura Pill */}
                <span
