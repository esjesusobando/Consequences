...
export default function App() {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Tab') {
        ...
        if (e.ctrlKey) {
          ...
          logMessage('info', `ATAJO DE TECLADO: Tab detectado. Panel DERECHO ${!hideRightPanel ? 'OCULTO' : 'VISIBLE'}`);
        }
      }
    };
  ...
  // Sync index.css root accents and custom Tokyo Cappuccino & HSL palettes
  useEffect(() => {
    const root = document.documentElement;

    // First, restore standard variables
    ...
    if (presentationConfig.themeMode === 'light_neocraft') {
      ...
      if (accent === 'custom') {
        ...
        root.style.setProperty('--accent-hex', accentsHexMap[accent as any] || '#156BFF');
      ...
      root.style.setProperty('--accent-hex', accentsHexMap[accent as any] || '#00F0FF');
    }
