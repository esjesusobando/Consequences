#!/usr/bin/env python3
"""
01_OS_Runtime_Test.py — PersonalOS v2.0 Consequences
...
def r17_skills_have_skill_md():
    """Cada subcarpeta de skills tiene al menos un SKILL.md."""
...
def r18_agent_sync_zero_drift():
    """Drift cero entre .agent/01_Agents y live."""
    ...
    if not only_live and not only_backup:
        return TestResult("R18", "Agent sync — drift", True,
                          f"Zero drift ({len(live_files)} archivos)")
...
def r19_root_structure_clean():
    """Raíz tiene exactamente las 4 carpetas v2.0."""
    ...
    if not missing:
        return TestResult("R19", "Root structure — 4 carpetas v2.0", True, "OK")
    return TestResult("R19", "Root structure — 4 carpetas v2.0", False,
                      f"Faltan: {missing}")
...
def r20_no_v1_paths_in_active_docs():
    ...
    for doc in active_docs:
        ...
        with open(doc, encoding="utf-8") as f:
            content = f.read()
        if "01_Personal_Os/01_Core/03_Skills/" in content or "01_Core/03_Skills/" in content or "08_Scripts_Os/" in content:
...
def main():
    print("\n" + "=" * 60)
    print("  OS RUNTIME TEST — PersonalOS v2.0 Consequences")
