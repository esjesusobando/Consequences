---
name: file-todos
description: This skill should be used when managing the file-based todo tracking system in the 03_Tasks/ directory. It provides workflows for creating todos, managing status and dependencies, conducting triage, and integrating with slash commands and code review processes.
---
...
The `03_Tasks/` directory contains a file-based tracking system for managing code review feedback, technical debt, feature requests, and work items. Each todo is a markdown file with YAML frontmatter and structured sections.
...
1. Determine next issue ID: `ls 03_Tasks/ | grep -o '^[0-9]\+' | sort -n | tail -1`
2. Copy template: `cp assets/todo-template.md 03_Tasks/{NEXT_ID}-pending-{priority}-{description}.md`
...
1. List pending items: `ls 03_Tasks/*-pending-*.md`
2. For each todo:
...
```bash
grep "^dependencies:" 03_Tasks/003-*.md
```

**To find what a todo blocks:**
```bash
grep -l 'dependencies:.*"002"' 03_Tasks/*.md
```

**To verify blockers are complete before starting:**
```bash
for dep in 001 002 003; do
  [ -f "03_Tasks/${dep}-complete-*.md" ] || echo "Issue $dep not complete"
done
```
...
5. Check for unblocked work: `grep -l 'dependencies:.*"002"' 03_Tasks/*-ready-*.md`
...
grep -l 'dependencies: \[\]' 03_Tasks/*-ready-p1-*.md

# List all pending items needing triage
ls 03_Tasks/*-pending-*.md

# Find next issue ID
ls 03_Tasks/ | grep -o '^[0-9]\+' | sort -n | tail -1 | awk '{printf "%03d", $1+1}'

# Count by status
for status in pending ready complete; do
  echo "$status: $(ls -1 03_Tasks/*-$status-*.md 2>/dev/null | wc -l)"
done
```
...
```bash
# Search by tag
grep -l "tags:.*rails" 03_Tasks/*.md

# Search by priority
ls 03_Tasks/*-p1-*.md

# Full-text search
grep -r "payment" 03_Tasks/
```

## Key Distinctions

**File-todos system (this skill):**
- Markdown files in `03_Tasks/` directory
